...
export default function AOOInteractiveMap() {
  ...
  const assignTeam = (buildingId: string, team: TeamNumber) => {
    setAssignments(prev => {
      ...
      if (team === null) {
        ...
        const teamBuildings = Object.entries(prev)
  ...
  const clearAll = () => {
    const cleared: Record<string, Assignment> = {};
    buildings.forEach(b => {
      cleared[b.id] = { team: null, order: null };
    });
  ...
  const moveOrder = (buildingId: string, direction: 'up' | 'down') => {
    ...
    setAssignments(prev => ({
      ...prev,
      [buildingId]: { ...prev[buildingId], order: prev[swapBuilding.id].order },
      [swapBuilding.id]: { ...prev[swapBuilding.id], order: prev[buildingId].order },
    }));
  };
            ...
            {selectedBuilding ? (
              ...
              <div className={`${theme.bgSecondary} rounded-xl p-4 border ${theme.border}`}>
                <p className={`text-sm ${theme.textMuted} text-center py-4`}>
                  Click a building on the map to assign it to a team
                </p>
              </div>
            )}
          </div>

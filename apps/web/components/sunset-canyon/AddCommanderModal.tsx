...
import { Commander, commanderDatabase, CommanderRarity } from '@/lib/sunset-canyon/commanders';
...
export function AddCommanderModal({ onAdd, onClose }: AddCommanderModalProps) {
    ...
    const filteredCommanders = commanderDatabase.filter((c) => {
                    ...
                    `}
                                    >
                                        <div className="flex items-center justify-between">
                                            ...
                                            <span className={`text-xs uppercase ${commander.rarity === 'legendary' ? 'text-yellow-500' : 'text-purple-400'
                                                }`}>
                                                {commander.rarity}
                                            </span>
                                        </div>
                            ...
                            {selectedCommander ? (
                                <div className="space-y-4">
                                    ...
                                    <div className="p-4 rounded-lg bg-stone-700/50 border border-stone-600">
                                        <h3 className={`text-lg font-semibold ${selectedCommander.rarity === 'legendary' ? 'text-yellow-500' : 'text-purple-400'
                                            }`}>
                                        ...
                                        <div className="flex gap-2">
                                            {[1, 2, 3, 4, 5].map((s) => (
                                                <button
                                                    key={s}
                                                    onClick={() => setStars(s)}
                                                    className={`p-2 rounded-lg transition-all ${stars >= s
